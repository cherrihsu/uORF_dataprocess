...
# # Please RUN USING LINUX OR UNIX SYSTEM
...
def run_MCEM(chains,params,maxiter=5,inner_loop=5*1000,positive_only=False,likelihood=None,args=None,vals=None,thr=None):
    pool = mp.Pool(chains)
    results = [pool.apply_async( MH_call, args = (np.random.uniform(0,1),maxiter+(ih+1),inner_loop+(ih+1),params,positive_only,likelihood,args,vals,thr) ) for ih in range(chains)]
...
for name in ["213","308","222"]:
    ...
    k = 10
    thr = 1.0e-10
    ks, sd = run_MCEM(f,9,f,k*f,positive_only=True,likelihood=custom_likelihood,args=true_vals[name],thr=thr)
